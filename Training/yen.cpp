...
struct Path {
	vector<VT> node;
	vector<VT> block;
	LT len;    //The index of the deviation node. Nodes before this node are on the
	//k shortest paths' tree.
	VT dev;    //Initialize a path with only one vertex.
	
	Path(VT v = 0) : node(), block() { node.push_back(v); len = 0; }
	
	bool operator > (const Path& p) const {
			return len > p.len || len == p.len && lexicographical_compare(p.node.rbegin(), p.node.rend(),node.rbegin(), node.rend() );
	}
};
...
public:
    ...
    void dijkstra() {
        ...
        for (int iter = 0; iter < m_verCnt; iter++) {
            ...
            for (int i = 0; i < m_verCnt; i++) {
                if (!m_visit[i] && ( -1 == minV || m_sh[i] < m_sh[minV] )) {
                    minV = i;
                }
            }
            if (-1 == minV) { break; }
    ...
    LT shortest(VT v) const { return m_sh[v]; }private:
    ...
    bool m_visit[MAX_VER];//Help the dijkstra.    //Help to make acyclic paths.
...
void solve() {
    ...
    if (kSh.size() < g_k) { cout << "None" << endl; }
		...
		//freopen("enjoy.in", "r", stdin);
		//freopen("enjoyOff.out", "w", stdout);
    while ( input() ) {
        solve();
    }
    return 0;
